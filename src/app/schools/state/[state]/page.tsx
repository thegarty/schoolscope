...
export async function generateMetadata({ params }: StatePageProps): Promise<Metadata> {
  ...
  const stateName = STATE_NAMES[state as keyof typeof STATE_NAMES]
  ...
  return {
    ...
    alternates: {
      canonical: `/schools/state/${state.toLowerCase()}`
    }
  }
}
...
export default async function StateSchoolsPage({ params, searchParams }: StatePageProps) {
  ...
  // Get filter options for this state
  const [types, sectors, suburbs] = await Promise.all([
    ...
    db.school.findMany({ 
      ...
      select: { suburb: true }, 
      orderBy: { suburb: 'asc' }
      // No limit - we'll handle this in the search component
    })
  ])
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mx-auto">
              ...
              <div className="text-center">
                <div className="text-3xl font-bold">{schools.reduce((sum, s) => sum + s._count.children, 0).toLocaleString()}</div>
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-12 prose max-w-none">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-3">
            Popular School Types in {stateName}
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 not-prose">
            {types.slice(0, 6).map((type) => (
              <Link 
                key={type.type}
                href={`/schools/state/${state.toLowerCase()}?type=${encodeURIComponent(type.type)}`}
                className="block p-4 border rounded-lg hover:bg-gray-50 transition-colors"
              >
                <div className="font-medium text-gray-900">{type.type} Schools</div>
                <div className="text-sm text-gray-500">Browse {type.type.toLowerCase()} schools in {stateName}</div>
              </Link>
            ))}
          </div>
        </section>
